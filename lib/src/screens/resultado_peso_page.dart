...
class _ResultadoPesoPageState extends State<ResultadoPesoPage> {
  ...
  Future<void> getPeso() async {
    try {
      final response = await http.get(Uri.parse('http://10.0.2.2:3000/peso')); // Cambia la IP según tu caso
    ...
    body: isLoading
        ...
        : error.isNotEmpty
            ...
            : ListView.builder(
                ...
                // Filtro para excluir registros donde el peso sea 0 o "0"
                itemCount: datos.where((item) {
                  final peso = double.tryParse(item['peso'].toString()) ?? 0;
                  return peso > 0; // Excluir pesos iguales a 0
                }).length,
